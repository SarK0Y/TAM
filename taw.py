...
def makeSharedFile(fileName: str):
    ...
    hfile=win32file.CreateFileW( file,\\
...
class PIPES:
    def __init__(self, outNorm, outErr):
        ...
        well_encoded_path = f"{outNorm}".encode(encoding=codepage)
        hfile = makeSharedFile(well_encoded_path)
        print(f"hfile = {hfile}")
...
def GetFileNames(fileName: str):
    ...
    err_out = f"{path}\\err_{name}".encode(encoding=GetWindowsCodePage())
